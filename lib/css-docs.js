;(function() {
    ...
    _.extend(CSSDocs.prototype, {
        init: function() {
            ...
            var block_tpl_content = fs.readFileSync(path.join(this.templatePath, 'doc-block.hbs'));
            var nav_tpl_content = fs.readFileSync(path.join(this.templatePath, 'navigation.hbs'));
            var layout_tpl_content = fs.readFileSync(path.join(this.templatePath, 'layout.hbs'));
        ...
        pageExists: function(dotPath) {
            return _(this.blocks).pluck('styleguide').contains(dotPath);
        },
